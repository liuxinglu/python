...
def update_bullets(game_settings, bullets, aliens, screen, ship, init_time):
    bullets.update()
    # 超出屏幕边界移除子弹
    # 子弹编组副本
    ...
    check_bullet_alien_collisions(game_settings, screen, ship, aliens, bullets, init_time)


def check_bullet_alien_collisions(game_settings, screen, ship, aliens, bullets, timeslop):
    """检查是否有子弹击中外星人"""
    """如果是，删除相应的子弹和外星人"""
    # 遍历aliens编组和bullets编组，如果有rect重叠，返回的字典种添加一个键值对
    # 第一个True表示碰撞后删除子弹，第二个True表示碰撞后删除外星人

    temp_time = time.time() - timeslop

    print(temp_time)

    for bullet in bullets:
        for alien in aliens:
            result = pygame.sprite.collide_rect(alien, bullet)
            if result:
                if alien.health == 0:
                    alien.kill()
                    bullet.kill()
    ...
    if len(aliens) == 0:
        bullets.empty()
        create_fleet(game_settings, screen, ship, aliens, _health_)
...
def ship_hit(game_settings, stats, screen, ship, aliens, bullets):
    ...
    if stats.ships_left > 0:
        ...
        aliens.empty()
        bullets.empty()
        create_fleet(game_settings, screen, ship, aliens, 0)
        ship.center_ship()
        sleep(0.5)
    else:
...
def create_alien(game_settings, screen, aliens, alien_number, row_number, _health_):
    alien = Alien(game_settings, screen)
    alien.health = _health_
...
def create_fleet(game_settings, screen, ship, aliens, _health_):
    alien = Alien(game_settings, screen)
    ...
    create_alien(game_settings, screen, aliens, 1, 1, _health_)
